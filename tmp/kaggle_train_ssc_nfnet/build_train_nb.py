...
Outputs: ssc_nfnet.onnx, val_preds.csv, train_log.json
"""
import json
...
def run_stage(X,Y,epochs,lr,tag,best,bestP):
    ...
    for ep in range(epochs):
        ...
        if auc>best: best=auc; bestP=P; torch.save(model.state_dict(),'/kaggle/working/ssc_nfnet_best.pt')
    return best,bestP
...
model.load_state_dict(torch.load('/kaggle/working/ssc_nfnet_best.pt')); model.eval()
torch.onnx.export(model,torch.randn(1,1,NMELS,TFR,device=DEV),'/kaggle/working/ssc_nfnet.onnx',
                  input_names=['mel'],output_names=['logits'],dynamic_axes={'mel':{0:'n'},'logits':{0:'n'}},opset_version=17)
print('WROTE ssc_nfnet.onnx, val_preds.csv  best=%.5f'%best)
'''
...
out = Path(__file__).parent / "train-ssc-nfnet.ipynb"

...
Outputs: ssc_hardlabel.onnx, val_preds.csv, train_log.json
"""
import json
...
K_PER_CLASS=40                 # focal cap per class
EP_A=10; LR_A=1e-3             # Stage A focal pretrain
EP_B=14; LR_B=4e-4             # Stage B soundscape fine-tune
...
for k in np.argsort(pf):
    ...
    XB.append(m.astype(np.float16)); YB.append((SOFT[k]>=0.3).astype(np.float32))
...
def run_stage(X,Y,epochs,lr,tag,best,bestP):
    ...
    for ep in range(epochs):
        ...
        if auc>best: best=auc; bestP=P; torch.save(model.state_dict(),'/kaggle/working/ssc_hardlabel_best.pt')
    return best,bestP
...
model.load_state_dict(torch.load('/kaggle/working/ssc_hardlabel_best.pt')); model.eval()
torch.onnx.export(model,torch.randn(1,1,NMELS,TFR,device=DEV),'/kaggle/working/ssc_hardlabel.onnx',
                  input_names=['mel'],output_names=['logits'],dynamic_axes={'mel':{0:'n'},'logits':{0:'n'}},opset_version=17)
print('WROTE ssc_hardlabel.onnx, val_preds.csv  best=%.5f'%best)
'''
...
out = Path(__file__).parent / "train-ssc-hardlabel.ipynb"
